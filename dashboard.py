...
with tabs[2]:
    ...
    if filtered_data.empty:
        ...
        if time_group == "Daily":
            ...
            trans_time['date'] = pd.to_datetime(trans_time['timestamp'])
            x_col = 'date'
        elif time_group == "Weekly":
            trans_time = filtered_data.resample('W-Mon', on='timestamp')['order_id'].count().reset_index()
            trans_time.rename(columns={'order_id':'transactions'}, inplace=True)
            x_col = 'timestamp'
        else:  # Monthly
            trans_time = filtered_data.resample('M', on='timestamp')['order_id'].count().reset_index()
            trans_time.rename(columns={'order_id':'transactions'}, inplace=True)
            x_col = 'timestamp'

        fig = px.line(trans_time, x=x_col, y='transactions',
...
with tabs[3]:
    ...
    top_n = st.slider("Top items to show:", 5, 30, 10, key="top_n_items")
    fig2 = px.bar(item_freq.head(top_n), x=item_freq.head(top_n).index, y=item_freq.head(top_n).values,
                  ...
                  template="plotly_white", key="top_items")
    st.plotly_chart(fig2, use_container_width=True)

    st.subheader("📈 Item Popularity Over Time")
    selected_item = st.selectbox("Select an item to see trends:", item_cols_filtered, key="select_item_trend")
    ...
    fig_item_trend = px.line(item_trend, x=item_trend.index.astype(str), y=item_trend.values,
                             labels={'x':'Month','y':'Transactions'}, title=f"{selected_item} Monthly Trend",
                             template="plotly_white", key="item_trend")
...
with tabs[4]:
    ...
    fig_top_buyers = px.bar(top_buyers, x=top_buyers.index, y=top_buyers.values,
                            labels={'x':'CustomerID','y':'Number of Transactions'}, title="Top 10 Buyers",
                            template="plotly_white", key="top_buyers")
...
with tabs[5]:
    ...
    monthly_sales = filtered_data.resample('M', on='timestamp')['order_id'].count()
    fig4 = px.line(monthly_sales, x=monthly_sales.index, y=monthly_sales.values,
                   ...
                   template="plotly_white", key="monthly_trends")
...
with tabs[7]:
    ...
    selected_items = st.multiselect("Select items in your basket:", options=item_cols_filtered, key="basket_select")
    min_conf = st.slider("Minimum confidence", 0.0, 1.0, 0.3, key="min_conf")
    min_lift = st.slider("Minimum lift", 0.0, 5.0, 1.0, key="min_lift")
...
with tabs[8]:
    ...
    selected_item_combo = st.selectbox("Select an item to see popular bundles:", options=item_cols_filtered, key="combo_select")
    top_n_bundles = st.slider("Top bundles to show:", 3, 15, 5, key="top_bundles")
    ...
    if not co_occurring_counts.empty:
        ...
        fig_bundle = px.bar(
            ...
            template="plotly_white", key="bundle_chart"
        )
